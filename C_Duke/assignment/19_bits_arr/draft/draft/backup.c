...
void numToBits(uint32_t *nums, int nNums, int *bits, int nBits) {
  ...
  for (size_t i = 0; i < nNums; i++) {
    for (int j = 31 + i * 32, counter = 31; j >= 0 + i * 32 && counter >= 0;
         j--, counter--) {
      bits[j] = getNthBit(nums[i], counter);
    }
  }
}
...
int main(void) {
  puts("here's me test");
  uint32_t number = 1;
  printf("%d's 1st bit is %d\n", number, getNthBit(number, 0));
  printf("%d's 31st bit is %d\n", number, getNthBit(number, 31));
  puts("test done");
